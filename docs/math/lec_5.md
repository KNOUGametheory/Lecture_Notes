...
- 백기 3개, 청기 2개, 홍기 4개를 나열하여 신호를 구분하고자 한다.
몇 가지 신호를 만들 수 있는가?
...
- 핵심만 쏙쏙!
	...
	> - 게임 적용
	>
	>	- 전략형 게임에서 유한 개의 행동(예를 들어, $${\bf L}eft, {\bf R}ight$$)을 갖는 
경기자의 혼합전략은
확률 $$p$$를 이용하여 $$p\times L+(1-p)\times R$$로 표현
	>
	>	- 전개형 게임에서 자연선택으로 게임이 어떤 시작 점(Node)에서 출발하는 지가 확률로서 결정
	>
...
각 근원사건 $$e_{i}$$가 \uline{일어날 가능성이 동일한 경우}에, $$k$$개의 원소로 구성된 사건 $$A$$가 일어날 확률은 다음과 같다.
	>
	>$$P(A)=\dfrac{k}{n}$$
