...
- 예제 1: $$𝑛$$이 홀수이면, $$𝑛^{2}$$도 홀수이다?
  ...
  - 논리 발전 및 결론: $$𝑄(𝑛)$$가 성립함을 도출
	...
	$$
	𝑛^2=2𝑘^′+1 \blacksquare
	$$

<!-- 예제 2: 실수 𝑎, 𝑏, 𝑥, 𝑦에 대하여,(𝑎^2+𝑏^2 )(𝑥^2+𝑦^2 )≥(𝑎𝑥+𝑏𝑦)^2?
‘(좌변) ≥ (우변)’임을 보이려면? (좌변) - (우변) ≥ 0!
(𝑎^2+𝑏^2 )(𝑥^2+𝑦^2 )−(𝑎𝑥+𝑏𝑦)^2=(𝑎^2 𝑥^2+𝑎^2 𝑦^2+𝑏^2 𝑥^2+𝑏^2 𝑦^2 ) 		−(𝑎^2 𝑥^2+2𝑎𝑏𝑥𝑦+𝑏^2 𝑦^2 )=𝑎^2 𝑦^2−2𝑎𝑏𝑥𝑦+𝑏^2 𝑥^2=(𝑎𝑦−𝑏𝑥)^2≥0
	(단, 등호(=)는  𝑎/𝑥=𝑏/𝑦일 때 성립) ∎ -->
...
대우 증명을 적용하려면?
대우 ¬𝑞→¬𝑝의 관점에서는 직접 증명과 동일
논리를 발전시키는 방향이 달라지면서 증명이 용이해지기도?

예제 1: 3𝑛+2가 홀수이면 𝑛은 홀수이다?
전제 ‘3𝑛+2가 홀수’에서 출발하면?3𝑛+2=2𝑘+1을 만족하는 정수 𝑘가 존재이항하여 3𝑛+1=2𝑘를 이용하려면? 경우 나누기? (how?)
대우를 이용하여 증명하면?명제의 대우: “𝑛이 짝수이면 3𝑛+2도 짝수이다”𝑛=2𝑘를 만족하는 정수 𝑘가 존재하면3𝑛+2=3(2𝑘)+2=2(3𝑘+1), 3𝑘+1∈ℤ ∎

예제 2: 자연수 𝑎와 𝑏에 대하여,𝑛=𝑎𝑏이면 𝑎≤√𝑛 또는 𝑏≤√𝑛?
직접 증명하려면?𝑛=𝑎𝑏로부터 유도할 수 있는 조건은?인수분해 후 각 인수에 대한 대소관계를 비교하려면?
대우를 이용하여 증명하면?명제의 대우: “(𝑎>√𝑛)∧(𝑏>√𝑛)→𝑛≠𝑎𝑏”두 부등식을 같은 변끼리 곱하면, 𝑎𝑏>√𝑛 √𝑛=𝑛 ∎ -->
...
𝑝: ‘참’임을 증명하고자 하는 정리
¬𝑝→𝑞가 ‘참’이 되게 하는 모순 𝑞를 발견
조건문 ¬𝑝→𝑞가 ‘참’이고 모순 𝑞가 ‘거짓’이면¬𝑝는 ‘거짓‘일 수밖에 없으므로 𝑝가 ‘참‘이라는 결론
모순 증명은 언제 활용할까?
직접 증명으로 증명하기 어려운 경우
모순을 발견하는 게 상대적으로 용이한 경우

예제 1: √2는 무리수이다?
무리수(irrational number)의 정의는?분수꼴 (𝑏⁄𝑎, 𝑎≠0, 𝑏는 서로소인 정수)로 표현될 수 없는 수분수꼴로 표현되지 않는다는 사실을 활용하려면?
모순을 이용하여 증명하면?√2가 유리수라고 가정 √2=𝑏⁄𝑎인 서로소인 정수 𝑎≠0,𝑏가 존재√2=𝑏⁄𝑎  𝑏=√2 𝑎  양변을 제곱하면 𝑏^2=2𝑎^2… (이하 생략) …

예제 2: 3𝑛+2가 홀수이면 𝑛은 홀수이다?
정리 𝑝→𝑞가 ‘거짓’이라 전제하려면?𝑝가 ‘참’이고 𝑞는 ‘거짓’!
여기에서부터 논리를 발전시켜 모순에 도달3𝑛+2가 홀수이고 𝑛은 짝수라고 가정𝑛=2𝑘를 만족하는 정수 𝑘가 존재 3𝑛+2=3(2𝑘)+2=2(3𝑘+1)이므로 짝수3𝑛+2가 홀수이고 동시에 3𝑛+2가 짝수? 모순! ∎
“𝑝→𝑞가 거짓”이라는 전제가 ‘거짓’  𝑝→𝑞가 참! -->
