...
-   들어가기
    ...
    > -   다음을 해보자.
    > 	
    >     -   모델 라이브러리에서 자신의 관심사 또는 전공 분야와 관련된 모형을 찾아보자.
...
-   들어가기
    ...
    > -   이 강의를 마친 후 다음을 할 수 있다.
    > 	
    >     -   ODD 프로토콜을 사용하는 이유를 설명할 수 있다.
    >		
    >     -   나비 모형을 통해 ODD 프로토콜의 내용을 이해할 수 있다.
    >	
    > -   다음을 해보자.
    > 	
    >     -   모형으로 분석해보고 싶은 사회 현상을 생각해보자.
    > 	
    >     -   생각한 사회 현상을 ODD 프로토콜로 바꾸어보자.
...
-   들어가기
    ...
    >     -   Netlogo의 행위자는 무엇인가?
    >		
    >     -   Netlogo 행위자의 변수 설정은 어떻게 하는가?
    >	
    >     -   Netlogo의 기본 문법은 무엇인가?
    >
    > -   다음을 해보자.
    > 	
    >     -   나비 모형의 ODD 프로토콜을 따라 앞으로 해야 하는 작업을 정리해보자.
    > 	
    >     -   간단한 모형을 만들어보자.
...
5. 넷로고의 행위자는 `turtles`, `patches`, `links`, `observer`이다.

6.  `setup`, `ask`, `ifelse` 등을 사용해 ODD 프로토콜을 기반으로 작동하는 간단한 모형부터 만들자.

7.  필요에 따라 변수 정의를 할 수 있고, 넷로고 코드의 끝은 `]` 또는 `end`로 마감해야한다.
