...
-   모델 라이브러리
    ...
    -   장점: 실현 $$\rightarrow$$ 선배 연구자들이 세운 가정과 수식이 어떻게 작동하는지 보여줌
...
-   나비가 곧장 언덕을 오를 확률 $$q$$ $$\rightarrow$$ 통로의 폭에 영향을 줄 읽기 자료 중것 $$\rightarrow$$ 이 둘의 관계를 그래프로 그려보자
...
-   주석 처리: 주석 처리하고 싶은 부분의 앞에 `;`, 편집기의 단축키를 사용하면 편함
...
-   통로 폭을 계산하자
    ...
    -   우리는 나비가 어떤 `patches` 를 사용할 지 모름
        ...
        -   초기 설정 변수(initializing variables): 새로운 변수는 다른 값을 지정하기 전에는 $$0$$에서 시작. 프로그램 작동에는 문제를 일으키지 않지만, 분석에서는 문제가 될 수 있는 데, 알기도 찾기도 어려운 에러가 됨 $$\rightarrow$$ 값을 주고 시작하는 습관을 들이도록
...
-   이제 통로 폭(corridor width)을 계산해야 함 $$\rightarrow$$ 값의 보고 `to-report` 사용
