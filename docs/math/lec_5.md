...
- (명제 1)
	...
	- $$A$$와 $$A^{c}$$는 항상 상호배반이고 $$A\cup A^{c}=S$$이므로
	$$P(S)=P(A\cup A^{c})=P(A)+P(A^{c})=1$$이다. 따라서,
	$$$$P(A)=1-P(A^{c})$$$$
	
	- 공정한 주사위를 던져 홀수가 나오는 사건을 $$A$$라고 하자.
	
	$$P(A)=\frac{3}{6}=\frac{1}{2}~~\text{$$\rightarrow$$}~~P(A^{c})=1-\frac{1}{2}=\frac{1}{2}$$
	
	  ![동전주사위 던지기](/images/Lec_5_5_2_3.pdf)
