...
- 반대칭적(antisymmetric)
  ...
  - 대칭적 관계와 반대칭적 관계는 서로 배타적? <span style="color:#FF0000">NO!</span>
...
- 추이적/전이적(transitive)
  ...
  - 집합 $$ 𝐴 = \{1, 2, 3, 4\}$$에 대한 $$ 𝑅 = \{(1, 2), (2, 1),(3, 4), (4, 3)\}$$은? 
     ...
     - 추이적 <span style="color:#FF0000">NO! (why?)</span> 
...
- 함수의 표현
  ...
  - 집합 $$ 𝐴 = \{1, 2, 3, 4\} $$에서 정의된 관계 $$ 𝑅 = \{(1, 2), (2, 4), (3, 4), (4, 3)\}$$은 함수인가?
    ...
    - <span style="color:#105AD2">YES!</span>
...
- 볼록결합(convex combination)
  ...
  - 계수 $$ 𝑎_{1}, 𝑎_{2}, \cdots , 𝑎_{𝑛}$$이 다음을 만족하는 선형결합
    ...
    $$
    𝑎_{1} + 𝑎_{2} + \cdots + 𝑎_{𝑛} = \sum_{i=1}^{n} a_{i} = 1	
    $$

    $$
    𝑎_{1} \geq 0, 𝑎_{2} \geq 0, \cdots, 𝑎_{𝑛} \geq 0 \Leftrightarrow \forall 𝑎_{𝑖} \geq 0, \quad 𝑖 = 1, 2, \cdots, 𝑛
    $$
...
- 함수의 정의를 활용한 그래프의 판별
  ...
  - 관계를 정의할 때 앞에 위치한 집합의 모든 원소가 하나의 관계에 해당하는 경우
	...
	![x^2](/images/Lec_3_x_2.png)
	
- 함수의 그래프와 함수가 아닌 것의 그래프

    ![arctan](/images/Lec_3_arctan_x.png)
	
    ![circle](/images/Lec_3_circle.png)
	
    ![sinh](/images/Lec_3_sinh.png)
	
    ![ncircle](/images/Lec_3_n_circle.png)


### 함수의 증가와 감소

- 점에서 함수의 <span style="color:#105AD2">증가상태</span>와 <span style="color:#FF0000">감소상태</span>

  - 함수 $$𝑓(𝑥)$$가 $$ 𝑥 = 𝑎$$에서 <span style="color:#105AD2">증가상태</span>
  ...
  - 함수 $$𝑓(𝑥)$$가 $$𝑥 = 𝑎$$에서 감소상태
    ...
    ![increasing](/images/Lec_3_i.png)
	
    ![decreasing](/images/Lec_3_d.png)

- 구간에서 함수의 증가와 감소

  - 함수 $$𝑓(𝑥)$$가 열린구간 $$(𝑎, 𝑏)$$에서 <span style="color:#105AD2">증가(increasing)</span>

    - 임의의 $$𝑥_{𝑖} < 𝑥_{𝑗} \in (𝑎,𝑏)$$에 대하여, $$𝑓(𝑥_{𝑖} )  < 𝑓(𝑥_{𝑗} ) $$
	
  - 함수 $$𝑓(𝑥)$$가 열린구간 $$(𝑎, 𝑏)$$에서 <span style="color:#FF0000">감소(decreasing)</span>

    - 임의의 $$ 𝑥_{𝑖} < 𝑥_{𝑗} \in (𝑎,𝑏) $$ 에 대하여, $$𝑓(𝑥_{𝑖}) > 𝑓(𝑥_{𝑗}) $$

    ![increasing](/images/Lec_3_increasing.png)
	
    ![decreasing](/images/Lec_3_decreasing.png)


- 구간에서 함수의 오목과 볼록

  - 함수 $$𝑓(𝑥)$$가 닫힌구간 $$ [𝑎, 𝑏] $$에서 아래로 <span style="color:#105AD2">볼록(convex)</span>
    ...
    ![convex](/images/Lec_3_convex.png)

	
  - 함수 $$𝑓(𝑥)$$가 닫힌구간 $$[𝑎, 𝑏]$$에서 <span style="color:#FF0000">오목(concave)</span>
    ...
    ![concave](/images/Lec_3_concave.png)
  ...
  - 실수 전체 집합에서 함수 $$𝑓(𝑥) = 𝑥^{2}$$의 볼록성 증명
    ...
    $$
    \begin{align}
    (\text{좌변}) & = 𝑓(\lambda 𝑥_{𝑖} + (1−\lambda) 𝑥_{𝑗} ) = [ \lambda 𝑥_{𝑖} + (1−\lambda) 𝑥_{𝑗} ]^{2}  \\
    (\text{우변}) & = \lambda𝑓(𝑥_{𝑖}) + (1−\lambda) 𝑓(𝑥_{𝑗}) = \lambda(𝑥_{𝑖})^{2} + (1−\lambda)(𝑥_{𝑗} )^{2} \\
    (\text{우변}) – (\text{좌변}) & = [ \lambda(𝑥_{𝑖})^{2} + (1−\lambda)(𝑥_{𝑗})^{2} ] − [ \lambda 𝑥_{𝑖} + (1−\lambda) 𝑥_{𝑗} ]^{2} \\
                                            & = [ \lambda(𝑥_{𝑖})^{2} + (1−\lambda) (𝑥_{𝑗})^{2} ] - [ \lambda^{2}(𝑥_{𝑖})^{2} + 2 \lambda(1−\lambda) 𝑥_{𝑖} 𝑥_{𝑗} + (1−\lambda)^{2} (𝑥_{𝑗})^2 ] \\
                                            & = \lambda(1 − \lambda)(𝑥_{𝑖})^{2} − 2 \lambda (1−\lambda) 𝑥_{𝑖} 𝑥_{𝑗} + \lambda(1−\lambda)(𝑥_{𝑗})^{2} \\
                                            & = \lambda(1 − \lambda) [(𝑥_{𝑖})^{2} − 2 𝑥_{𝑖} 𝑥_{𝑗} + (𝑥_{𝑗})^{2} ] \\
                                           & = \lambda(1 − \lambda) [𝑥_{𝑖} − 𝑥_{𝑗}]^{2} \geq 0 \\	
    \end{align}
    $$

  - 실수 전체 집합에서 함수 $$𝑓(𝑥 ) =𝑥^{2} $$의 그래프

    ![x_2](/images/Lec_3_x_2.png)
...
- 그래프를 활용하여 수요와 공급의 균형을 구하면?
  ...
  - 수요함수와 공급함수의 그래프를 작성하고 교점의 좌표를 구하면 
    ...
    ![equilibrium](/images/Lec_3_equilibrium.png)
